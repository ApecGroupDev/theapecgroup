...
const ServicesBoxLinks: React.FC = () => {
  ...
  // Filter out the compliance item only on /team
  const filteredFeatures = pathname === "/services/compliance"
    ? features.filter(feature => feature.slug !== "compliance")
    : features;
  return (
    <div>
      {/* Feature Boxes */}
      <div className="flex flex-row mt-auto scrn-1000:pb-0 gap-4 scrn-1000:gap-6 scrn-1400:gap-8 scrn-2000:gap-12">
        {filteredFeatures.map((feature, index) => (
          <Link key={index} href={`/services/${feature.slug}`} passHref>
            <div
              className="relative group bg-gray-100 p-3 scrn-1000:p-6 rounded-lg shadow-lg hover:shadow-xl hover:bg-white transition-all duration-500 
              w-20 scrn-1000:w-24 scrn-1400:w-32 scrn-1900:w-40 scrn-2000:w-48"
            >
              {/* Image Display */}
              <div className="flex items-center justify-center 
                h-12 scrn-1000:h-12 scrn-1400:h-16 scrn-1900:h-24 scrn-2000:h-32"
              >
                <img
